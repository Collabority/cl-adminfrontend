...
const EditService = () => {
    ...
    <div className="bg-white rounded-lg shadow flex flex-col gap-6 p-4">
      ...
      <div className="flex flex-col items-center gap-4 border-3 border-dashed border-gray-300 hover:border-[#1447E6] transition-all duration-300 p-8 sm:p-10 md:p-12 rounded-md text-center w-full">
        ...
        {formData?.coverImage && typeof formData.coverImage === "string" && (
          <img src={formData.coverImage} alt="Preview" className="mt-2 max-h-20 rounded-lg" />
        )}
        {formData?.coverImage instanceof File && (
          <img src={URL.createObjectURL(formData.coverImage)} alt="Preview" className="mt-2 max-h-20 rounded-lg" />
        )}
      </div>
    </div>
  );

  if (!formData) return <div className="p-4 text-center font-semibold">Loading service...</div>;
  ...
  return (
    <form className="flex flex-col gap-6 p-4">
      ...
      <p className="text-gray-600 font-semibold text-base">Update the details below to edit and publish your service.</p>
