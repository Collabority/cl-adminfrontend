...
//     if (!name.trim() || !title.trim() || !company.trim() || !reviewContent.trim() || !reviewTitle.trim() || !category.trim() || rating === 0) {
//       alert('Please fill in all required fields.');
//       return;
//     }

//     // Bind all data in FormData
...
//       .then((response) => {
//         alert(`Review ${finalStatus.toLowerCase()} successfully!`);
//         // navigate("/reviews");
//       })
//       .catch((error) => {
...
//           <button onClick={() => navigate(-1)} className="border border-gray-300 rounded-lg px-4 sm:px-5 py-2 text-gray-700 font-semibold bg-white shadow-sm hover:bg-gray-50 transition text-base w-full sm:w-auto">Cancel</button>
...
//             </select>
//             <button className="ml-auto bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-5 py-2 font-bold transition text-base shadow-sm w-full sm:w-auto" onClick={handleSaveReview}>Save Review</button>
//           </div>
//         </div>
//       </div>
//     </div>
...
const reviewSchema = z.object({
  ...
  reviewTitle: z.string().min(3, "Review title is too short"),
...
const AddReview = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      alert(`Review ${data.status.toLowerCase()} successfully!`);
      navigate("/reviews");
    } catch (error) {
      console.error("Error saving review:", error);
      alert("Failed to save review. Please try again.");
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow p-6 max-w-4xl mx-auto">
        ...
        <div className="my-6">
          ...
          <input type="file" ref={fileInputRef} onChange={handleProfilePicChange} className="hidden" />
          <button
            type="button"
          ...
          {profilePicUrl && <img src={profilePicUrl} alt="Preview" className="mt-2 h-20 rounded" />}
        </div>
        ...
        <div className="my-6">
          ...
          <input {...register("reviewTitle")} className="w-full border p-2 rounded" />
          {errors.reviewTitle && <p className="text-red-500 text-sm">{errors.reviewTitle.message}</p>}
        </div>

        <div className="my-6">
          <label>Review Content *</label>
          <textarea {...register("reviewContent")} className="w-full border p-2 rounded min-h-[100px]" />
          {errors.reviewContent && <p className="text-red-500 text-sm">{errors.reviewContent.message}</p>}
        </div>
