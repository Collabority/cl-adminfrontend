...
const AddNewSubscriber = ({ onSave, onCancel, isLoading }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const capitalizedStatus = subscriptionStatus.charAt(0).toUpperCase() + subscriptionStatus.slice(1);

    const fullPayload = {
      ...formData,
      name: `${formData.firstName} ${formData.lastName}`.trim(), 
      status: capitalizedStatus, 
      segment: segments.length > 0 ? segments[0] : "General", 
    ...
    <div className="p-4 sm:p-6 md:p-8 bg-gray-50 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4 px-2 sm:px-4 pt-6 pb-2">
        ...
        >
          ...
          <span className="text-black font-bold">Add New Subscriber</span>
        </button>
      </div>

      <div className="p-4 flex-col gap-6">
        <h1 className="text-xl font-semibold">Add New Subscriber</h1>
        <p className="font-medium text-base text-gray-500">
          Add individual subscribers or import them in bulk to grow your mailing
          list.
        </p>
        ...
        <div className="p-6 flex gap-6 items-center justify-end bg-white rounded-lg shadow-md">
          <button 
            type="button" 
            onClick={onCancel} 
            className="p-2 border rounded-md border-gray-500 min-w-[100px]"
          >
            Cancel
          </button>
          ...
          <button 
            ...
            disabled={isLoading}
            className="bg-blue-500 p-2 rounded-md text-white font-medium min-w-[150px] disabled:bg-blue-300"
          >
            {isLoading ? "Saving..." : "Add Subscriber"}
          </button>
        </div>
      </div>
    </div>
